...
tt = t_min / 60.0          # 小时
# 以区间起点画阶梯图, 更直观: 区间 k 对应 [k/6, (k+1)/6) 小时
...
fig.tight_layout(); fig.savefig('output/fig1_price_load_pv.png', dpi=130)
plt.close(fig)
...
fig.tight_layout(); fig.savefig('output/fig2_netload_purchase.png', dpi=130)
plt.close(fig)
...
fig.tight_layout(); fig.savefig('output/fig3_buy_charge_discharge.png', dpi=130)
plt.close(fig)
...
fig.tight_layout(); fig.savefig('output/fig4_storage_energy.png', dpi=130)
plt.close(fig)

print('图片已生成: output/fig1~fig4.png')
