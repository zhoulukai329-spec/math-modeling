"""Fill Q2 report tables from the solution and evaluation CSV files."""
import csv
import re
...
def fmt(value, decimals=4):
    value = float(value)
    return f"{0.0 if abs(value) < 0.5*10**(-decimals) else value:.{decimals}f}"
...
def main():
    ...
    mc = csv_rows("monte_carlo_results.csv")
    ...
    plan_table = table(["日期"]+[f"{h}:00-{h}:10" for h in [10, 12, 14, 16, 18, 20]]
                       +["全天计划购电量 (kWh)", "全天计划购电费 (元)"], plan_rows)
    charge_table = table(["日期"]+[b+" 充电/放电" for b in blocks]+["0:00 储电量", "24:00 储电量"], charge_rows)
    ...
    base_rows = [[r["策略"]]+[r[k] for k in ["计划费_元", "紧急费_元", "总成本_元"]] for r in baselines]
    base_paper = [row+[f'{(float(r["总成本_元"])/base-1)*100:+.2f}%'] for row, r in zip(base_rows, baselines)]
    forecast_table = table(["方法", "MAE (kW)", "RMSE (kW)", "偏差 (kW)"],
                           [[r["方法"]]+[fmt(r[k], 2) for k in ["MAE_kW", "RMSE_kW", "Bias_kW"]] for r in forecasts])
    ...
    robust_table = table(["检验项", "结果"], [
        ...
        ["插补敏感性（相对主方案最大差异）", f"输出期总成本差 {delta:.2f} 元（{delta/sens_base*100:.6f}%）；基于 CSV 两位小数精度"],
        ["蒙特卡洛 日成本 5%/50%/95% 分位", " / ".join(mc[0][k] for k in ["5%分位", "50%分位", "95%分位"])+" 元"],
        ["蒙特卡洛 应急电量 5%/50%/95% 分位", " / ".join(mc[1][k] for k in ["5%分位", "50%分位", "95%分位"])+" kWh"]])
    source = "> 数据来自本次运行的 prob2_solution.npz 与检验 CSV；能量单位为 kWh，费用单位为元，合计按未舍入值计算。"
    for filename, paper in [("问题2_求解报告.md", False), ("问题2_求解报告_论文版.md", True)]:
        path = dio.OUTPUT_DIR/filename
        text = path.read_text(encoding="utf-8")
        text = re.sub(r"(?m)^> .*?(?:待填|待重跑).*?$", source, text)
        ...
        if paper:
            ...
            text = replace_table(text, "### 7.6", table(["策略", "计划费(元)", "紧急费(元)", "总成本(元)", "相对无储能因果策略"], base_paper))
            ...
            text = replace_table(text, '#### 怎么公平', table(["策略", "计划费(元)", "紧急费(元)", "总成本(元)"], base_rows))
