"""Causal point forecasts and residual PV scenarios reused by Question 4-3."""
...
def build_pv_scenarios(
    ...
    scenarios = np.maximum(0.0, np.stack([point + paths[int(i)] for i in chosen]))
    chosen_days = np.asarray([source_days[int(i)] for i in chosen], dtype=int)
    return (scenarios, chosen_days) if return_source_days else scenarios
