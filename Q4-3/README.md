...
- PV预报在发布后1小时生效，首小时沿用上一轮预测，历史残差遵循相同规则；跨日同样处理。
  数据起点无上一轮预测时使用零光伏启动先验；虚拟前瞻超过24小时仍沿用末值。
- 负荷预测沿用Q2：优先使用上周同一真实时间戳，前7天历史不足时使用已揭示同槽均值。
- 未来电价依次使用上周同一时段、已发生日期同一时段均值和首日边界先验预测。
- 光伏与价格误差场景使用相同历史来源日，并沿真实时间轴跨到下一业务行，绝不在午夜
  回到同一来源行开头；完全重复的联合场景自动合并概率。
