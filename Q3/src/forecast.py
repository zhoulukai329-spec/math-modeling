...
from data_io import DT_HOURS, InputData, coerce_date, parse_clock_minutes, power_kw_to_energy
...
def _causal_load_profile(data: InputData, issue_day: date) -> np.ndarray:
    """Mean only completed operating days; current/future actuals are excluded."""
    eligible = [i for i, day in enumerate(data.dates) if day < issue_day]
    if not eligible:
        return np.zeros(144, dtype=float)
    return data.load_energy[np.asarray(eligible)].mean(axis=0)
...
def build_information_forecast(
    ...
    profile = _causal_load_profile(data, issue_day)
...
def build_pv_scenarios(
    ...
    last_required_offset = start_step + horizon_steps - 1
    days_required = last_required_offset // 144 + 1
    first = max(0, issue_day_index - lookback_days)
    candidates = [day for day in range(first, issue_day_index) if day + days_required <= issue_day_index]
    if not candidates:
        paths = [np.zeros(horizon_steps, dtype=float)]
    else:
        paths = [_historical_path(residuals, day, start_step, horizon_steps) for day in candidates]
    rng = np.random.default_rng(seed + issue_day_index)
    chosen = rng.choice(len(paths), size=n_scenarios, replace=len(paths) < n_scenarios)
    return np.stack([point + paths[int(i)] for i in chosen])
